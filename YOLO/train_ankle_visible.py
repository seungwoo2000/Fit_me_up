...
def train(device):
    ...
    yaml_path   = os.path.join(BASE, 'data_ankle_visible.yaml')
    test_source = os.path.join(BASE, 'YOLO_ankle_visible_Labeling', 'split_data', 'test', 'images')
    ...
    print(f"\n⏱ ankle_visible_only_{device_tag} 총 소요시간: {int(elapsed//3600)}시간 {int((elapsed%3600)//60)}분 {int(elapsed%60)}초")
    return elapsed
